...
export default function ProfileClient({
  ...
  const [collaborators, setCollaborators] = useState<User[]>(
    initialCollaborationData?.collaborators || [],
  );
  const [sentRequests, setSentRequests] = useState<CollaborationRequest[]>(
    initialCollaborationData?.sentRequests || [],
  );
  const [receivedRequests, setReceivedRequests] = useState<
    CollaborationRequest[]
  >(initialCollaborationData?.receivedRequests || []);
  const [currentUserId, setCurrentUserId] = useState(
    initialCollaborationData?.currentUserId,
  );
  const [currentSpaceId, setCurrentSpaceId] = useState(
    initialCollaborationData?.currentSpaceId,
  );
  ...
  const [largeLimit, setLargeLimit] = useState<string>(
    String(initialSettings?.largeAmountLimit || "5000"),
  );
  const [reminderEnabled, setReminderEnabled] = useState<boolean>(
    initialSettings?.backupReminder?.enabled || false,
  );
  const [isUpdating, setIsUpdating] = useState<boolean>(false);

  // Export States
  const [exportScope, setExportScope] = useState<"month" | "range" | "all">(
    "month",
  );
  ...
  const handleRequestAction = async (
    ...
    try {
      const data = await secureFetch<{ message: string }>(
  ...
  const getBackupTime = () => {
    ...
    const timePart =
      parts.length > 1 ? formatDisplayTime(settings.lastBackupAt) : "";
    ...
    <AppShell>
      ...
      <div className="space-y-8">
          ...
          <div className="card-surface p-4 space-y-4">
            <form onSubmit={handleInvite} className="flex gap-2">
              ...
              <Button
                disabled={isInviting}
                size="icon"
                className="h-12 w-12 shrink-0 active:scale-95 transition-all"
                  ...
                  >
                    ...
                    <div className="flex gap-2 shrink-0">
                      <Button
                        ...
                        className="active:scale-95 transition-all h-8 px-3 text-xs"
                      >
                        Accept
                      </Button>
                      <Button
                        ...
                        className="active:scale-95 transition-all h-8 px-3 text-xs bg-transparent"
                      >
                        Reject
                      </Button>
                    </div>
            ...
            >
              ...
              {sentRequests.map((req) => (
                <div
                  key={req._id}
                  className="flex items-center justify-between gap-3 p-2 opacity-60"
                >
                  <div className="flex items-center gap-3 min-w-0">
                    <div
                      className="tile w-10 h-10 shrink-0"
        ...
        <section>
          ...
          <div className="card-surface p-5 space-y-5">
            ...
            <Button
              ...
              className="w-full h-12 font-bold active:scale-95 transition-all"
              style={{ background: "var(--primary)", color: "white" }}
            >
              {isUpdating ? (
        ...
        <section>
          ...
          <div
            className="card-surface overflow-hidden divide-y"
            style={{ borderColor: "var(--border)" }}
          >
            <div className="p-4 space-y-4">
              ...
              {exportScope === "range" && (
                <div
                  className="grid grid-cols-2 gap-3.5 p-3.5 rounded-2xl"
                  style={{ background: "var(--surface-muted)" }}
                >
            ...
            <button
              ...
              className="flex items-center gap-3 px-4 py-3 w-full active:scale-95 transition-all group"
            >
              <div
                className="tile w-10 h-10 shrink-0"
              ...
              <ChevronRight
                className="ml-auto w-4 h-4 group-hover:translate-x-1 transition-transform"
                style={{ color: "var(--muted-foreground)" }}
              />
            </button>
            <button
          ...
          <div className="card-surface p-4 flex flex-col gap-4">
            <div className="flex items-center justify-between gap-4">
              ...
              <Button
                ...
                variant="outline"
                size="sm"
                className="shrink-0 h-9 font-bold active:scale-95 transition-all bg-transparent"
                style={{
                  borderColor: "var(--border)",
                  color: "var(--foreground)",
                }}
      ...
      <ConfirmDialog
        ...
        title={
          removeConfirm?.userId === currentUserId
            ? "Leave Space?"
            : "Remove Collaborator?"
        }
        description={
        ...
        confirmText={
          removeConfirm?.userId === currentUserId ? "Leave Now" : "Remove Now"
        }
        variant="destructive"
      />
    </AppShell>
